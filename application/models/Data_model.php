...
function get_by_kesimpulan_freq($kesimpulan,$periode,$idpertanyaan){
	$bulan = date('n',strtotime($periode."-1"));
	$tahun = date('Y',strtotime($periode."-1"));
	$sql = "select count(b.bobot) as total
			from data a 
			inner join data_jawaban b on a.id=b.id_data
			where year(a.datetime)='$tahun' and 
					month(a.datetime)='$bulan' and 
...
function get_by_bobot_freq($kesimpulan,$periode,$idpertanyaan){
	$bulan = date('n',strtotime($periode."-1"));
	$tahun = date('Y',strtotime($periode."-1"));
	$sql = "select count(b.bobot) as total
			from data a 
...
function get_by_kesimpulan_count($periode,$idpertanyaan){
	$bulan = date('n',strtotime($periode."-1"));
	$tahun = date('Y',strtotime($periode."-1"));
	$sql = "select count(b.bobot) as total
			from data a 
...
function get_by_kesimpulan($id,$date){
	$bulan = date('n',strtotime($date."-1"));
	$tahun = date('Y',strtotime($date."-1"));
	$select=array(
				'data_jawaban.id_pertanyaan',
				'avg(data_jawaban.bobot) as rata'
			);
	$where=array(
				'month(data.datetime)'=>$bulan,
				'year(data.datetime)'=>$tahun,
...
function get_by_responden($periode,$idpertanyaan){
	$bulan = date('n',strtotime($periode."-1"));
	$tahun = date('Y',strtotime($periode."-1"));
	$select=array(
				'data_jawaban.jawaban',
...
function get_by_responden_detail($periode,$idpertanyaan,$jawaban){
	$bulan = date('n',strtotime($periode."-1"));
	$tahun = date('Y',strtotime($periode."-1"));
	$select=array(
				'data_jawaban.jawaban',
